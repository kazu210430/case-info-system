...
### foreground outcome decision contract freeze line

foreground outcome chain は decision contract であり、completion owner ではありません。`CompleteForegroundGuaranteeOutcome(...)` が terminal outcome を作っても、`case-display-completed` は `TryCompleteCreatedCaseDisplaySession(...)` 相当の existing completion chain が one-time emit 条件を満たした場合だけ emit します。

固定する意味:

- foreground decision は completion emit owner ではありません。
- foreground execution result は `case-display-completed` ではありません。
- foreground outcome は completion input にはなり得ますが、display session boundary、visibility terminal、pane visible、created CASE reason を代替しません。
- visibility recovery outcome と foreground outcome を統合しません。`visible / refresh success`、`foreground required / not required`、`foreground succeeded / degraded` は別軸です。
- `WindowActivate` dispatch / downstream observation が foreground trace fields を持っても、dispatch そのものを foreground success / completion に読み替えません。

decision contract truth:

| status | freeze line 上の decision contract | success / failure への丸め禁止 |
| --- | --- | --- |
| `RequiredSucceeded` | foreground guarantee が必要で、execution が attempted かつ recovered。display-completable terminal。 | `case-display-completed` そのものではない。 |
| `RequiredDegraded` | foreground guarantee が必要だったが、success guarantee まではできなかった。display-completable terminal。 | success へ読み替えない。failure へも雑に丸めない。 |
| `NotRequired` | foreground guarantee が不要、または required path の事実が揃わない。display-completable terminal。 | foreground success ではなく、foreground path 非該当として読む。 |

現時点では foreground runtime extraction、service 新設、semantics 変更を行いません。R13 を Phase 5 で扱う場合も、R10/R11/R12 visibility / source / rebuild outcome、R14 display session completion、R15 WindowActivate downstream observation との距離を維持した protocol-preserving shrink に限定します。

### Phase 4 合格基準

- foreground decision / outcome / trace owner は現時点では `TaskPaneRefreshOrchestrationService` に残っていること。
