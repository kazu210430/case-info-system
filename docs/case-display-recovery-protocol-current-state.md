...
- 同じ orchestration service には、workbook-open 直後など window 依存 refresh が成立しない場合の fallback handoff と timer fallback が残り、ready retry scheduling は `TaskPaneReadyShowRetryScheduler` へ委譲されています。
...
- ready 条件が不足していれば `TaskPaneReadyShowRetryScheduler` 側の retry schedule へ戻り、`TaskPane wait-ready retry scheduled` / `TaskPane wait-ready retry firing` が plain log として残ります。
...
| --- | --- | --- | --- |
| ready retry `80ms` | `TaskPaneReadyShowRetryScheduler` | retry timer firing 後に ready-show attempt を再実行する。attempt 2 でも window resolve は `activateWorkbook: true` で走るが、`WorkbookWindowVisibilityService.EnsureVisible(...)` は attempt 1 のみ。 | retry scheduler owner は scheduler。activation は window resolve の副作用。final foreground guarantee owner ではない。 |
...
| save-before-close normalization | `KernelCaseCreationService.NormalizeInteractiveWorkbookWindowStateBeforeSave(...)` / `NormalizeBatchWorkbookWindowStateBeforeSave(...)` | interactive は hidden create session 内で save 前に owned workbook window を `Visible=true` へ戻さず、必要なら `xlNormal` のみ整える。batch は save 前に `Visible=true` と `xlNormal` を担保する。必要なら `workbook.Activate()` / `NewWindow()` で save 用 window を確保する。 | 保存状態正規化。shared/current app の CASE foreground guarantee ではない。 |
...
- ready-show retry owner と visibility primitive owner が混ざりやすい。
  - ready retry `80ms` は `TaskPaneReadyShowRetryScheduler` が担う。
...
- refresh / retry / completion owner:
  - ready-show attempt: `WorkbookTaskPaneReadyShowAttemptWorker`
  - ready retry scheduler: `TaskPaneReadyShowRetryScheduler`
  - pending retry / created-case display session / visibility outcome / foreground outcome / `case-display-completed`: `TaskPaneRefreshOrchestrationService`
...
- ready-show retry owner とは別である。
  - ready retry `80ms` は `TaskPaneReadyShowRetryScheduler` と `TaskPaneDisplayRetryCoordinator` が担う。
