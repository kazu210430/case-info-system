...
- この制約は実機確認に基づく安定化契約です。future reviewer は「保存前 visible + normal」だけを根拠に interactive hidden create session へ `Visible=true` を戻してはなりません。
...
### 会計書類セット close の UI 制御

会計フォーム / import prompt の close lifecycle は `docs/accounting-close-lifecycle-current-state.md` を正本とします。現コードと実機安定化済みの順序を維持し、helper 化・共通化・再整理の対象にしません。

- Excel の × 印 close と、フォーム上の「Excelを閉じる」ボタン経由 close は同一視しません。
- 対象 workbook に active な会計フォームまたは import prompt がある場合、Excel の × 印 close は form-button allow flag が無い限り cancel し、フォームボタンから閉じるよう案内します。
- フォームボタン経由 close では、先に form / prompt を `Close()` / `Dispose()` し、handlers / references / 黄色セル cleanup を完了してから直 `workbook.Close()` を呼びます。
- `Application.Quit()` は、フォームボタン経由で対象 workbook を閉じた後、同じ `Application` の `Workbooks.Count == 0` を確認できた場合だけ実行します。
- `AccountingImportRangePromptForm` と `AccountingReverseGoalSeekForm` の Form class へ `OnFormClosing` gate を戻しません。`FormClosing` / `FormClosed` cleanup は service 側の event handler が担います。
