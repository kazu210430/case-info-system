...
- hidden-for-display の presentation handoff 直前に必要な shared app state facts、previous window restore decision、visible-open window facts の組み立ては `CaseWorkbookPresentationHandoffService` が担当します。これは `CaseWorkbookOpenStrategy` から表示復旧判断・材料化を切り出したものであり、`Application.Visible`、`WindowState`、activate、workbook open / close、CASE 表示 recovery 実行 owner は移動していません。
...
| --- | --- | --- | --- | --- |
| hidden create save normalization | `KernelCaseCreationService` | hidden create session の save 前。`NormalizeInteractiveWorkbookWindowStateBeforeSave(...)` / `NormalizeBatchWorkbookWindowStateBeforeSave(...)` | interactive は `ResolveOrCreateWorkbookWindowForSave(...)` の後も `window.Visible = true` を実行せず、必要なら `WindowState = xlNormal` だけを整える。batch は保存状態正規化として `window.Visible = true` と minimized 時の `WindowState = xlNormal` を行う。 | `NewCaseVisibilityObservation` の `save-window-*`。interactive では `save-window-visible-deferred` を記録する。 |
