...
- `dev/CaseInfoSystem.ExcelAddIn/App/CreatedCaseDisplaySessionStateReader.cs`
- `dev/CaseInfoSystem.ExcelAddIn/App/TaskPaneRefreshEmitContext.cs`
- `dev/CaseInfoSystem.ExcelAddIn/App/TaskPaneRefreshEmitPayloadBuilder.cs`
- `dev/CaseInfoSystem.ExcelAddIn/App/TaskPaneRefreshCompletionDecisionService.cs`
...
| R14 | `BeginCreatedCaseDisplaySession(...)` / `TryCompleteCreatedCaseDisplaySession(...)`。session 読み取りは `CreatedCaseDisplaySessionStateReader`、completion 判定分類は `TaskPaneRefreshCompletionDecisionService`、emit payload 材料整形は `TaskPaneRefreshEmitContext` / `TaskPaneRefreshEmitPayloadBuilder` | display protocol session boundary | ready-show acceptance で start、ready-show callback or refresh path 終端で completion。 | はい。最重要 coordinator 残存領域。 | owner / lifecycle は不可。読み取り・判定・payload material の追加固定は可。 | 高 | 低から中 | 現時点では owner / lifecycle の service 化はしない。 | はい | いいえ | はい |
...
### Phase 5 runtime intermediate boundary summary

Phase 5 の中間 helper note と、その後の completion / emit / session collaborator 分離は、下の `Phase 5 runtime closure boundary` を正本とする boundary progression として扱います。詳細な現在地は `docs/taskpane-display-recovery-current-state.md` の closure note と `Completion / emit / session boundary current-state` を参照します。

| checkpoint | boundary summary | owner 移動 |
| --- | --- | --- |
| completion decision collaborator | `TaskPaneRefreshCompletionDecisionService` を追加。R14 hard gate decision と `Blocked` / `SessionMissing` / `ReadyToEmit` 分類を局所化。 | なし。R14 completion callback 発火 / emit / session owner は残る。 |
| emit context / payload collaborator | `TaskPaneRefreshEmitContext` / `TaskPaneRefreshEmitPayloadBuilder` を追加。`case-display-completed` payload material と diagnostic details assembly を局所化。 | なし。emit 発火責務 / 発火タイミング / trace emit position は残る。 |
| session state reader | `CreatedCaseDisplaySessionStateReader` を追加。session start / lookup / snapshot DTO 化だけを局所化。 | なし。session lifecycle、dictionary / lock / `IsCompleted` mark / remove は残る。 |
| third runtime | R10/R11/R12 normalized outcome chain helper を追加。R10 -> R11 -> R12 の順序だけを局所化。 | なし。normalized outcome は completion owner ではない。 |
| R13 classification | foreground execution result classification helper を追加。`RequiredSucceeded` / `RequiredDegraded` の分類だけを局所化。 | なし。foreground execution / trace / completion owner は残る。 |
| R13 trace details | foreground trace details helpers を追加。details 文字列 assembly のみを局所化。 | なし。trace emit / foreground execution / completion owner は残る。 |
| foreground display-completable | R14 hard gate が読む foreground input 判定 helper を追加。 | なし。completion owner / emit owner / session owner は残る。 |

中間 checkpoint と collaborator 分離はいずれも owner / lifecycle 移動ではなく、completion callback 発火、`case-display-completed` emit、session lifecycle、one-time emit guard、WindowActivate handling、callback / pending semantics を持ちません。

中間 checkpoint と collaborator 分離の STOP / freeze line は closure boundary に集約します。completion callback 発火、emit 発火、session lifecycle、session dictionary / lock / `IsCompleted` mark / remove、route semantics、callback meaning、R07/R09/R13/R14 横断 extraction は引き続き STOP です。

### Phase 5 runtime closure boundary

Phase 5 runtime は、R13 foreground display-completable input helper 化後の時点で一度区切ります。

ここまでで完了した safe unit は、owner / lifecycle を動かさずに切れる読み取り・材料整形・判定範囲に限定されています。

| completed safe unit | closure 上の意味 | owner 移動 |
| --- | --- | --- |
| `CreatedCaseDisplaySessionStateReader` | session start / lookup / snapshot DTO 化を局所化。 | なし。session lifecycle、dictionary / lock / `IsCompleted` mark / remove は残る。 |
| `TaskPaneRefreshCompletionDecisionService` | completion hard gate の yes/no decision と result status 分類を局所化。 | なし。completion callback 発火責務 / 発火タイミングは残る。 |
| `TaskPaneRefreshEmitContext` / `TaskPaneRefreshEmitPayloadBuilder` | completion trace details / payload material assembly を局所化。 | なし。emit owner / trace emit owner / session owner は残る。 |
...
完了済み collaborator / helper 群は、completion callback 発火責務、emit 発火責務、emit 発火タイミング、session lifecycle、callback meaning、pending retry semantics、WindowActivate semantics、trace contract を持ちません。
