...
Phase 5 の中間 runtime note と、その後の completion / emit / session collaborator 分離は、次の `Phase 5 runtime closure note` と `Completion / emit / session boundary current-state` を正本とする進行履歴に要約します。いずれも owner 移動ではありません。

| checkpoint | 追加された collaborator / helper | 維持する contract |
| --- | --- | --- |
| completion decision collaborator | `TaskPaneRefreshCompletionDecisionService` | R14 completion callback 発火責務、発火タイミング、session lookup、one-time guard は未移動。 |
| emit context / payload collaborator | `TaskPaneRefreshEmitContext` / `TaskPaneRefreshEmitPayloadBuilder` | emit 発火責務、発火タイミング、`NewCaseVisibilityObservation.Complete(...)`、trace emit position は未移動。 |
| session state reader | `CreatedCaseDisplaySessionStateReader` | session lifecycle、dictionary / lock / `IsCompleted` mark / remove、one-time guard は未移動。 |
...
続く safety-preserving shrink として、observation 後の normalized outcome chain 接続、foreground 実行要否、retry continuation の判断は `TaskPaneRefreshObservationDecisionService` と `TaskPaneRefreshRetryContinuationDecisionService` に分離します。これは owner / lifecycle の移動ではありません。retry timer、WindowActivate event owner、ready-show callback、completion / emit / created-case session owner、TaskPane 表示命令は orchestration 側に残します。目的は巨大 orchestration class の retry / observation 判断責務を減らし、変更影響範囲を小さくすることです。

ここまでで owner / lifecycle を動かさずに切れる読み取り・材料整形・判定 collaborator は現時点で概ね取り切りました。残る runtime 候補は、completion callback 発火責務、emit 発火責務、display session lifecycle、callback meaning、route semantics、trace contract に近く、ここから先を runtime 実装として進めると protocol-preserving refactor ではなく semantics rewrite になりやすい領域です。

Phase 5 runtime 完了済み collaborator / helper:

1. `CreatedCaseDisplaySessionStateReader` による created-case display session の start 判定、lookup 判定、snapshot DTO 化。
2. `TaskPaneRefreshCompletionDecisionService` による R14 completion hard gate decision と `Blocked` / `SessionMissing` / `ReadyToEmit` result 分類。
3. `TaskPaneRefreshEmitContext` / `TaskPaneRefreshEmitPayloadBuilder` による `case-display-completed` emit payload 用材料整形。
4. R10/R11/R12 normalized outcome chain helper。
5. R13 foreground classification helper。
6. R13 foreground trace details helpers。
7. R13 foreground display-completable input helper。

完了済み collaborator / helper 群の性質:

- owner 移動はありません。
- completion callback 発火責務を持ちません。
- completion callback 発火タイミングを決めません。
- emit 発火責務を持ちません。
- emit 発火タイミングを決めません。
...
- retry owner / retry timer lifecycle。
- ready-show 実行責務。
- foreground guarantee 実行責務。
- completion callback 発火責務と発火タイミング。
- emit 発火責務と発火タイミング。
- created-case display session lifecycle。
- session dictionary / lock / `IsCompleted` mark / remove。
- route semantics / callback meaning。
- UI 表示順序。
- 表示回復そのものの実行責務。
...
### Completion / emit / session boundary current-state

この節は、直近で完了した TaskPane refresh completion / emit / session 境界分離後の current-state 契約です。作業履歴ではなく、今後の変更判断に使う設計契約として読む。

#### `TaskPaneRefreshOrchestrationService` に残す責務

`TaskPaneRefreshOrchestrationService` は TaskPane refresh の全材料を抱える場所ではなく、owner / lifecycle / orchestration を保持する場所です。

残す責務:

- refresh 本線の順序制御。
- retry owner。
- retry timer lifecycle。
- ready-show 呼び出し。
- foreground guarantee 呼び出し。
- completion callback 発火責務。
- emit 発火責務。
- created-case display session lifecycle owner。
- session dictionary / lock / `IsCompleted` mark / remove。
- route semantics / callback meaning の保持。
- UI 表示順序の保持。

collaborator 側へ分離済みの責務:

- created-case display session の読み取り / snapshot 化。
- emit payload 用の材料整形 / diagnostic material。
- completion 判定材料 / decision / result 分類。

この境界にした理由は、`case-display-completed` の one-time emit、created-case display session lifecycle、ready-show callback、retry / foreground / UI 表示順序が同じ protocol completion に収束するためです。材料整理だけを外へ出し、owner / lifecycle / 発火タイミングは orchestration に残す。

#### `CreatedCaseDisplaySessionStateReader`

`CreatedCaseDisplaySessionStateReader` は、created-case display session の状態を読むための collaborator です。

担当すること:

- start 判定。
- lookup 判定。
- `CreatedCaseDisplaySessionSnapshot` への DTO 化。
- orchestration 側が持つ session 状態を、completion / emit / diagnostic 用に読み取りやすくすること。
...
- session dictionary / lock / `IsCompleted` mark / remove。
- emit 発火。
- foreground guarantee / ready-show / retry。
- 表示回復そのものの実行。

#### `TaskPaneRefreshEmitContext` / `TaskPaneRefreshEmitPayloadBuilder`

`TaskPaneRefreshEmitContext` は emit payload の材料を束ねる context です。`TaskPaneRefreshEmitPayloadBuilder` は、その context から `case-display-completed` payload を構築します。

担当すること:

- emit payload 用の材料整形。
- workbook / window diagnostic descriptor の組み立て。
- created-case display session snapshot の emit 用変換。
- `case-display-completed` の details / observation payload 構築。
...
- emit 発火タイミングの決定。
- UI 操作。
- retry / foreground guarantee / ready-show。
- session lifecycle の開始・終了・消費。

#### `TaskPaneRefreshCompletionDecisionService`

`TaskPaneRefreshCompletionDecisionService` は completion 判定材料を整理し、decision / result / reason / status を返す collaborator です。

担当すること:

- completion 判定材料の整理。
- hard gate の reason / status 分類。
- session 解決前の `Blocked` / `ReadyForSession` decision。
- session 解決後の `Blocked` / `SessionMissing` / `ReadyToEmit` result 分類。
- foreground display-completable terminal input の読み取り。

担当しないこと:

- completion callback 発火。
- completion callback 発火タイミングの決定。
- retry。
- foreground guarantee / ready-show / emit。
- session dictionary / lock / `IsCompleted` mark / remove。
- route semantics / callback meaning の変更。

#### 明確な STOP 領域

今後の改修で次を collaborator 側へ移そうとする場合は STOP とする。実装に入る前に、別タスクとして設計判断を明示する必要がある。
...
- reader / context / decision / payload builder のテスト追加。
- reason / status / diagnostic material の既存挙動固定。
- orchestration 本体から、読み取り専用材料・snapshot・payload material をさらに外へ出す。
- ただし owner / lifecycle / callback / emit / retry / foreground / ready-show は動かさない。

注意が必要な方向:

- retry 周辺。
- foreground guarantee 周辺。
- ready-show lifecycle 周辺。
- completion callback の意味。
- emit 発火タイミング。
- created-case session lifecycle。

これらは安全確認だけで進めず、別タスクとして明示的な設計判断が必要な領域として扱う。
