...
| `experimental-shared` | 専用 hidden `Application` を生成 | なし | cleanup は `legacy-isolated` と同じ | route 名に反して current/shared app は使いません。差分は close 時の inner save だけです。 |
...
- `experimental-shared` は current/shared app を使わないため、正規仕様として追認せず削除候補または rename 候補として扱います。
