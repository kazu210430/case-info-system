...
| `KernelCaseCreationService.Normalize*WorkbookWindowStateBeforeSave(...)` | interactive は isolated session 内で `Visible=true` を強制せず、必要なら `WindowState=xlNormal` のみ整える。batch は save 前に `Visible=true`、必要なら `WindowState=xlNormal` | 保存ファイルへ minimized 状態を残さないための owner-side cleanup。interactive の visible 化は presentation service へ遅延し、display handoff ではない。 |
