...
| R06 | `ScheduleTaskPaneReadyRetry(...)` | retry / fallback ownership boundary | ready-show attempt 1 失敗後、attempt 2 を 80ms で schedule。 | いいえ。ただし順序 contract は orchestration が見る。 | はい。retry 値と cleanup 固定後。 | 低から中 | 低 | 候補。ready retry scheduler。 | いいえ | いいえ | いいえ |
...
| R15 | `WindowActivatePaneHandlingService` + orchestration downstream trace | display route / trigger observation boundary | WindowActivate dispatch 後、refresh entry の start / outcome。 | 部分的に必須。誤認防止 trace は近接が必要。 | はい。ただし observation contract 固定後。 | 中 | 中 | 候補。trigger observation object。 | 部分的 | いいえ | はい |
| R16 | `StopPendingPaneRefreshTimer(...)` / `StopWaitReadyRetryTimers(...)` | timer lifecycle boundary | success / shown callback / explicit stop。 | いいえ。 | はい。 | 低 | 低 | 候補。timer lifecycle owner。 | いいえ | いいえ | いいえ |
...
| R16 | cleanup は retry lifecycle と session completion の重複発火防止であり、Dispose helper ではない。 | R06/R08 の owner が固定される前に分けると timer leak / duplicate retry の責任が曖昧になる。 |
...
2. R16 timer lifecycle boundary
   no-op 条件が明確で、cleanup 自体は挙動を持ちにくいです。R06/R08 の retry owner 固定後なら安全度が高いです。

3. R06 ready-show retry timer
   `80ms` delay と attempt 2 発火に閉じた小さい境界です。値・順序・trace 名を固定した後なら分離候補です。
...
3. R06 の ready retry scheduler 固定。
