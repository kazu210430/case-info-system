...
- Shutdown 順序も固定します。現行 `main` では `UnhookApplicationEvents() -> StopPendingPaneRefreshTimer() -> KernelHomeForm close / null 化 -> TaskPaneManager.DisposeAll() -> word warm-up timer 停止 -> hidden Excel shutdown` の順で扱います。
...
- 現時点で code 側の最小候補として触ってよい境界は、`KernelHomeForm` のインスタンス管理だけです。
- 対象は `_kernelHomeForm` の生成、再利用、close、dispose、null guard の ownership に限定します。
...
- 候補は `KernelHomeForm` のインスタンス管理だけを host 化する案です。
- host 化してよいのは instance ownership だけで、呼び出し位置、suppression 判定、表示順序は変えません。
