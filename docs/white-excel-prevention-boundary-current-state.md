...
## Implementation delta (2026-05-12 post-close retry)

- `PostCloseFollowUpScheduler` は、`targetWorkbookStillOpen=True` を初回判定だけで terminal skip しない。
- close 直後に対象 workbook が `Application.Workbooks` に残って見える状態を想定し、`targetWorkbookStillOpen` の場合は 250ms 間隔で最大 5 回だけ再確認する。
- retry 後に queued target が消えた場合だけ visible workbook scan へ進み、visible workbook が無い場合だけ既存どおり `Application.Quit()` を試みる。
- retry 上限後も queued target が残る場合は `WhiteExcelPreventionNotRequired` / `outcomeReason=targetWorkbookStillOpenRetryExhausted` として quit しない。
- trace には `attemptNumber`、`targetStillOpenRetriesRemaining`、`excelBusyRetriesRemaining`、`workbooksCount`、`activeWorkbookPresent`、`applicationVisible`、retry schedule / timer schedule の理由を追加する。
- Excel busy retry は既存の別 retry として維持する。
- startup marker guard は、有効 marker があり、`/restore` 起動、workbooks count 0、active workbook なし、表示中の通常 workbook なし、open Kernel workbook なしの場合に限り、`Application.Visible=True` でも既存の遅延再確認へ進める。単純に `ApplicationVisible` 条件を外すものではない。
...
- `WhiteExcelPreventionNotRequired` は current-state では `targetWorkbookStillOpenRetryExhausted` と `visibleWorkbookExists` の 2 つの主な reason を持ちます。どちらも quit しない decision であり、display success ではありません。
...
- `targetWorkbookStillOpen` は queued key と current open workbook key が一致したことを表します。この場合は短時間 retry し、retry 上限後も残っている場合だけ `WhiteExcelPreventionNotRequired` として quit しません。
...
- immediate reopen 近接時は follow-up cancel ではなく、dequeue 時の fresh facts による still-open retry / retry-exhausted skip または visible workbook skip として読めます。
...
| target still-open check / retry | `PostCloseFollowUpScheduler` | queued request dequeue | queued workbook key と current `Application.Workbooks` enumeration から得た fresh open workbook key の比較。still-open なら 250ms x 最大 5 回 retry | `post-close-follow-up-decision` / `post-close-follow-up-retry-scheduled` |
...
- queue dequeue 後に target still-open check を行い、still-open なら短時間 retry します。
- retry 後に still-open でない場合だけ visible workbook scan へ進みます。
- retry 上限後も still-open の場合は quit しません。
- visible workbook がある場合は quit しません。
- visible workbook が無い場合だけ quit を試みます。

post-close follow-up 条件、visible workbook scan、quit 条件は維持します。`targetWorkbookStillOpen` のみ、初回 terminal skip から有限 retry 後の terminal skip へ変更済みです。
...
- immediate reopen により fresh workbook が queued key と一致した場合も、current-state では follow-up cancel ではなく still-open retry として扱います。retry 上限後も一致する場合は `WhiteExcelPreventionNotRequired` / `targetWorkbookStillOpenRetryExhausted` を記録します。
...
| `WhiteExcelPreventionNotRequired` / `targetWorkbookStillOpenRetryExhausted` | queued target が retry 上限後も current open workbook として残っているため quit 不要。 | `targetWorkbookStillOpen=True`、`quitAttempted=False`、`attemptNumber=6`、`targetStillOpenRetriesRemaining=0`。 | quit なし。 |
...
- immediate reopen 近接時の正式な follow-up cancel / reopen gating protocol は未定義です。現行は有限 still-open retry と visible workbook scan による fail-closed 判定です。
...
この節は G-0 docs-only 時点の次候補履歴です。2026-05-12 の post-close retry では、`targetWorkbookStillOpen` の初回 terminal skip だけを有限 retry に変更し、visible workbook scan / quit 条件は維持しました。

1. `PostCloseFollowUpScheduler` の existing emitted outcome と raw facts の意味を tests / docs で固定する。
2. `WhiteExcelPreventionNotRequired` の reason を `targetWorkbookStillOpenRetryExhausted` と `visibleWorkbookExists` で明確に assertion する。
...
- Excel busy retry 条件。target still-open retry は 2026-05-12 に 250ms x 最大 5 回として追加済み。
...
## G-0 docs-only で行わなかったこと

以下は G-0 docs-only 作業時点の履歴です。2026-05-12 の runtime 変更は上記 Implementation delta を正とします。
