...
- 現行 `main` の `dev/CaseInfoSystem.ExcelAddIn/ThisAddIn.cs` は 1510 行です。大型化していた 2187 行規模の状態から、startup / execution boundary と runtime diagnostics を外へ寄せることで行数削減と責務削減が進んでいます。
- 今回の分割は責務統合ではなく、VSTO add-in 境界に近い判断・facts / trace 組み立て・bridge 処理の collaborator 化です。
- `ThisAddIn` は VSTO event 受け口、composition root 呼び出し、Application event wiring / unwiring、CustomTaskPane create / remove の concrete adapter に寄りました。
