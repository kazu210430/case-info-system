...
### Phase 5 R13 classification runtime state boundary

Phase 5 R13 foreground classification helper 化後、runtime で完了済みの安全単位は次の 4 点です。
...
R13 classification helper 化後も未移動のもの:
...
次 runtime 候補はまだ GO ではありません。次に runtime を触る場合は、foreground trace details assembly helper 化の tests-first 評価を先に置きます。
