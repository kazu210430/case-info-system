...
- `DocumentExecutionPolicyService` は allowlist / review 関連の旧 runtime 互換コードです。
  - 現在は runtime 本線の配線から外れており、文書作成フローでは生成・使用されません。
  - allowlist / review の config ファイルと csproj 同梱設定は撤去済みです。専用 tools は撤去済みで、サービス本体は次フェーズの削除候補です。
...
- `DocumentExecutionPolicyService` のソースは残っていますが、現在は runtime 本線から外れており、次フェーズの削除候補です。
...
- `allowlist`
  ...
  - 過去の運用・検証用の残存要素です。
  - config ファイルと csproj 同梱設定は撤去済みです。専用 tools は撤去済みです。
  - 今後の段階的撤去候補として扱います。
- `review`
...
- `DocumentExecutionPolicyService`
  - allowlist / review の過去互換要素を含む残存サービスです。
  - 現在は runtime 本線から外れており、文書作成フローでは生成されません。
  - 将来の段階的撤去対象候補です。
