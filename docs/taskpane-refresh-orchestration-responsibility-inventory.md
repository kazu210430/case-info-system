...
- ready-show retry scheduler への委譲。
...
- created-case display session state reader への start / lookup / snapshot 判定委譲。
- completion decision service への hard gate 判定と `Blocked` / `SessionMissing` / `ReadyToEmit` 分類委譲。
- emit context / payload builder への diagnostic material と payload 構築委譲。
- `WindowActivate` downstream trigger trace。
- `case-display-completed` の唯一 emit。

巨大化した理由は、無関係な処理を雑に足したためだけではありません。むしろ、window が安定しない Office event lifecycle、hidden-for-display 後の ready-show、pane already-visible path、foreground guarantee、protection / suppression、観測 trace を同じ protocol completion に束ねる必要があり、安全装置がこの class に集まったためです。直近の分離では、session 読み取り、completion 判定分類、emit payload 材料整形だけを collaborator に出し、owner / lifecycle / callback / emit / retry / foreground / ready-show は orchestration に残しています。
...
| 項目 | 内容 |
| --- | --- |
| 現行 owner | `TaskPaneReadyShowRetryScheduler`。呼び出し口は `TaskPaneRefreshOrchestrationService.ShowWorkbookTaskPaneWhenReady(...)` に残り、timer lifecycle は `TaskPaneRetryTimerLifecycle` へ委譲する |
...
| trace 責務 | `wait-ready-retry-scheduled`、`wait-ready-retry-firing`、plain `TaskPane wait-ready retry scheduled/firing`。trace 名と意味は不変 |
...
| 変更理由 | retry delay / attempt logging / scheduler ownership |
| 将来変更されやすい点 | `80ms` と max attempts は contract truth として固定済み。変更する場合は freeze line 改定が必要 |
...
| 項目 | 内容 |
| --- | --- |
| 現行 owner | `TaskPaneRefreshOrchestrationService.BeginCreatedCaseDisplaySession(...)` / `TryCompleteCreatedCaseDisplaySession(...)`。start / lookup / snapshot 判定は `CreatedCaseDisplaySessionStateReader`、completion hard gate / result 分類は `TaskPaneRefreshCompletionDecisionService`、emit payload 材料整形は `TaskPaneRefreshEmitContext` / `TaskPaneRefreshEmitPayloadBuilder` |
...
- `CreatedCaseDisplaySessionStateReader` は start 判定、lookup 判定、snapshot DTO 化だけを担う。session lifecycle owner ではなく、session dictionary / lock / `IsCompleted` mark / remove、emit 発火、ready-show、retry、foreground guarantee、表示回復実行は持たない。
- `TaskPaneRefreshCompletionDecisionService` は completion 判定材料、hard gate reason / status、session 解決後の `Blocked` / `SessionMissing` / `ReadyToEmit` 分類だけを担う。completion callback 発火、発火タイミング、retry、foreground guarantee、ready-show、emit、session lifecycle、route semantics、callback meaning は持たない。
- `TaskPaneRefreshEmitContext` / `TaskPaneRefreshEmitPayloadBuilder` は emit payload 用材料整形、workbook / window diagnostic descriptor、session snapshot の emit 用変換、payload 構築だけを担う。emit 発火責務と発火タイミングは持たず、UI 操作、retry、foreground guarantee、ready-show、session lifecycle に触れない。
- `TaskPaneRefreshOrchestrationService` は completion callback 発火、emit 発火、session dictionary / lock / `IsCompleted` mark / remove、`NewCaseVisibilityObservation.Complete(...)`、route semantics、callback meaning、UI 表示順序を保持する。
...
| R06 ready retry scheduler | R06 safe unit で分離済み。scheduling は attempt 本体、fallback handoff、completion から分けられる | attempt 上限、80ms delay、timer cleanup owner は固定済み |
...
5. `CreatedCaseDisplaySessionStateReader`、`TaskPaneRefreshCompletionDecisionService`、`TaskPaneRefreshEmitContext` / `TaskPaneRefreshEmitPayloadBuilder` の reason / status / diagnostic material を tests で固定する。
6. R06 ready-show retry scheduler は safe unit として分離済み。R08 の pending retry state は、R07 fallback handoff 条件を固定したまま別途検討する。
7. R14 completion emit は最後に扱う。completion callback 発火、emit 発火、session lifecycle owner を移す場合は、別タスクとして設計判断を明示する。
