...
namespace CaseInfoSystem.ExcelAddIn.App
{
	internal sealed class KernelCaseCreationService
	{
		...
		internal KernelCaseCreationResult CreateSavedCase (Workbook kernelWorkbook, KernelCaseCreationPlan plan)
		{
			...
			bool keepNewCaseVisibilityObservationSession = false;
			try {
				...
				if (RequiresDeferredVisiblePresentationStartupState (plan.Mode)) {
					NewCaseVisibilityObservation.Begin (plan.Mode, plan.CaseWorkbookPath);
					NewCaseVisibilityObservation.Log (_logger, _excelInteropService, application, null, null, "create-enter", "KernelCaseCreationService.CreateSavedCase", plan.CaseWorkbookPath);
				}
				...
				keepNewCaseVisibilityObservationSession = RequiresDeferredVisiblePresentationStartupState (plan.Mode);
				...
				if (!keepNewCaseVisibilityObservationSession && RequiresDeferredVisiblePresentationStartupState (plan.Mode)) {
					NewCaseVisibilityObservation.Complete (plan.CaseWorkbookPath);
				}
			...
			Application application = fallbackApplication;
				try {
					...
					if (deferredVisiblePresentationRequired) {
						...
						NewCaseVisibilityObservation.Log (_logger, _excelInteropService, application, workbook, null, "save-window-before-save", "KernelCaseCreationService.CreateSavedCaseWithoutShowing", plan.CaseWorkbookPath, "scope=isolated,route=" + hiddenCaseWorkbookSession.RouteName + ",mode=interactive");
		...
		private void NormalizeInteractiveWorkbookWindowStateBeforeSave (Workbook workbook, KernelCaseCreationPlan plan, Stopwatch stopwatch, string routeName)
		{
			...
			try {
				...
				NewCaseVisibilityObservation.Log (_logger, _excelInteropService, null, workbook, window, "save-window-visible-deferred", "KernelCaseCreationService.NormalizeInteractiveWorkbookWindowStateBeforeSave", plan.CaseWorkbookPath, "scope=isolated,route=" + (routeName ?? string.Empty) + ",mode=interactive,visibilityChangeReason=defer-to-presentation,visibilityOwner=KernelCasePresentationService");
				if (window.WindowState != XlWindowState.xlNormal) {
					NewCaseVisibilityObservation.Log (_logger, _excelInteropService, null, workbook, window, "save-window-before-windowstate-normal", "KernelCaseCreationService.NormalizeInteractiveWorkbookWindowStateBeforeSave", plan.CaseWorkbookPath, "scope=isolated,route=" + (routeName ?? string.Empty) + ",mode=interactive,visibilityChangeReason=prepare-save-windowstate-normal,windowStateNormalizeRequested=xlNormalWhenNotNormal");
					window.WindowState = XlWindowState.xlNormal;
					NewCaseVisibilityObservation.Log (_logger, _excelInteropService, null, workbook, window, "save-window-after-windowstate-normal", "KernelCaseCreationService.NormalizeInteractiveWorkbookWindowStateBeforeSave", plan.CaseWorkbookPath, "scope=isolated,route=" + (routeName ?? string.Empty) + ",mode=interactive,visibilityChangeReason=prepare-save-windowstate-normal,windowStateNormalizeRequested=xlNormalWhenNotNormal");
				}
				NewCaseVisibilityObservation.Log (_logger, _excelInteropService, null, workbook, window, "save-window-after-normalize", "KernelCaseCreationService.NormalizeInteractiveWorkbookWindowStateBeforeSave", plan.CaseWorkbookPath, "scope=isolated,route=" + (routeName ?? string.Empty) + ",mode=interactive,visibilityChangeReason=prepare-save-windowstate-normal,visibilityChangeDeferred=True,visibilityOwner=KernelCasePresentationService,windowStateNormalizeRequested=xlNormalWhenNotNormal");
				NewCaseVisibilityObservation.Log (_logger, _excelInteropService, null, workbook, window, "save-window-normalized", "KernelCaseCreationService.NormalizeInteractiveWorkbookWindowStateBeforeSave", plan.CaseWorkbookPath, "scope=isolated,route=" + (routeName ?? string.Empty) + ",mode=interactive,visibilityChangeDeferred=True,visibilityOwner=KernelCasePresentationService,callerStep=KernelCaseCreationService.NormalizeInteractiveWorkbookWindowStateBeforeSave,windowStateNormalizeRequested=xlNormalWhenNotNormal");
		...
		private Window ResolveOrCreateWorkbookWindowForSave (Workbook workbook, string caseWorkbookPath, string routeName, string callerStep)
		{
			...
			if (SafeWorkbookWindowCount (workbook) > 0) {
				Window existingWindow = workbook.Windows [1];
				NewCaseVisibilityObservation.Log (_logger, _excelInteropService, null, workbook, existingWindow, "save-window-resolve-existing-window", callerStep, caseWorkbookPath, "scope=isolated,route=" + (routeName ?? string.Empty) + ",resolveAction=ExistingWindow");
