...
- `TaskPaneRefreshOrchestrationService.ScheduleTaskPaneReadyRetry(...)`
...
3. `TaskPaneRefreshOrchestrationService.ScheduleTaskPaneReadyRetry(...)` が `80ms` timer を登録します。
...
- `ScheduleTaskPaneReadyRetry(...)` の trace は `maxAttempts=2` を出しますが、attempt 3 を schedule / firing し得ます。
