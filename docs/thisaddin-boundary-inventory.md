...
- current-state 事実確認手順: `docs/current-state-review.md`
...
- startup 時に `VstoEventAdapter` 経由で Excel application event を購読する
- startup 時の HOME 表示判定、初回 `RefreshTaskPane("Startup", null, null)`、managed-close startup guard は `AddInStartupBoundaryCoordinator.RunAfterApplicationEventsHooked()` が起動する
- `ThisAddIn_Shutdown(...)` が `ShutdownCleanupAdapter.HandleShutdown()` を呼び、event unhook、pending pane refresh timer 停止、Kernel HOME form close、`TaskPaneManager.DisposeAll()`、word warm-up timer 停止、managed-close startup guard timer 停止、retained hidden app-cache cleanup (`ShutdownHiddenApplicationCache()`) を順に行う
...
3. startup boundary coordinator 初期化
   - `InitializeStartupBoundaryCoordinator()`
4. adapter / service 接続
   - `InitializeAdaptersAfterComposition()`
5. application event hook
   - `HookApplicationEvents()` が `VstoEventAdapter.SubscribeApplicationEvents()` を呼ぶ
6. startup HOME 判定 / 初回 refresh / managed-close startup guard
   - `AddInStartupBoundaryCoordinator.RunAfterApplicationEventsHooked()` が `KernelWorkbookService.ShouldShowHomeOnStartup()`、`RefreshTaskPane("Startup", null, null)`、managed-close startup guard を順に扱う

private helper 化や読みやすさ改善を行っても、この上位順序は動かさない前提で扱います。

### Application event wiring / unwiring

- `HookApplicationEvents()` は `VstoEventAdapter.SubscribeApplicationEvents()` を呼び、adapter 内の `ApplicationEventSubscriptionService.Subscribe()` が次の Excel event を既存順序で購読する
  - `WorkbookOpen`
  - `WorkbookActivate`
  - `WorkbookBeforeSave`
  - `WorkbookBeforeClose`
  ...
  - `SheetChange`
  - `AfterCalculate`
- `UnhookApplicationEvents()` は `VstoEventAdapter.UnsubscribeApplicationEvents()` を呼び、adapter 内の `ApplicationEventSubscriptionService.Unsubscribe()` が同じ event を解除する
- event handler 本体は `VstoEventAdapter` 側へ寄せ、`ThisAddIn` は hook / unhook の呼び出し位置と一部 bridge entry を残す
...
- Startup 順序は固定します。現行 `main` では `InitializeStartupDiagnostics() -> CreateStartupCompositionRoot() / ApplyCompositionRoot() -> InitializeStartupBoundaryCoordinator() -> InitializeAdaptersAfterComposition() -> HookApplicationEvents() -> AddInStartupBoundaryCoordinator.RunAfterApplicationEventsHooked()` の順で扱います。
- `RunAfterApplicationEventsHooked()` では、Kernel HOME 表示判定より前に `KernelStartupContextInspector` と `KernelWorkbookStartupDisplayPolicy` を通る startup context 判定を済ませます。
- Shutdown 順序も固定します。現行 `main` では `ShutdownCleanupAdapter.HandleShutdown()` の中で `UnhookApplicationEvents() -> StopPendingPaneRefreshTimer() -> KernelHomeFormHost.CloseOnShutdown -> TaskPaneManager.DisposeAll() -> StopWordWarmupTimer -> StopManagedCloseStartupGuardTimer -> retained hidden app-cache cleanup` の順で扱います。
...
- `AddInStartupBoundaryCoordinator.RunAfterApplicationEventsHooked()`、`ThisAddIn.ShowKernelHomePlaceholder()`、`HomeTransitionAdapter.HideKernelHomePlaceholder()` の呼び出し位置は変えません。
...
- `2026-05-17` の `main` / `origin/main` `616b4af88d15571f0e57a95f447693f0426faeb7` では、`dev/CaseInfoSystem.ExcelAddIn/ThisAddIn.cs` は 645 行です。これは確認時点のスナップショットであり固定仕様ではありません。大型化していた 2187 行規模の状態から、startup / execution boundary と runtime diagnostics を外へ寄せることで行数削減と責務削減が進んでいます。
- 今回の分割は責務統合ではなく、VSTO add-in 境界に近い判断・facts / trace 組み立て・bridge 処理の collaborator 化です。
- `ThisAddIn` は VSTO Startup / Shutdown 入口、composition root 呼び出し、Application event hook / unhook の呼び出し位置、adapter delegate surface、CustomTaskPane create / remove adapter に寄りました。Excel event handler 本体は `VstoEventAdapter` 側へ寄っています。

### Coordinator / diagnostics service に移った責務

- `AddInStartupBoundaryCoordinator`
  - startup HOME decision handoff
  - startup refresh handoff
  - managed-close startup guard facts / delay decision / timer
  - empty-startup quit decision
  - `DisplayAlerts` quit bridge
- `AddInExecutionBoundaryCoordinator`
  - document-action `ScreenUpdating` execution bridge
  - TaskPane refresh suppression count
  - suppression enter / exit trace
- `AddInRuntimeExecutionDiagnosticsService`
  - runtime execution diagnostic detail building
  - assembly location / version / hash / build marker trace

### ThisAddIn に残す owner

- VSTO `Startup` / `Shutdown` 入口
- composition root 呼び出しと依存 field の適用
- Application event wiring / unwiring の呼び出し位置
- Application event handler 入口
- Workbook / Window event の coordinator への dispatch
- `CustomTaskPane` create / remove の concrete VSTO adapter
- shutdown cleanup 呼び出し順序
- Kernel HOME 表示入口と TaskPane 表示要求入口

### ThisAddIn に戻してはいけない責務

- startup guard facts / empty-startup quit decision / `DisplayAlerts` quit bridge
- document-action `ScreenUpdating` bridge
- TaskPane refresh suppression count の直接管理
- runtime execution diagnostics detail building
- hidden Excel lifecycle / retained app-cache cleanup owner
- TaskPane refresh completion / emit owner
- workbook close owner、app quit owner、COM release owner

次候補は、`ThisAddIn` に残る event handler glue のさらなる整理です。ただし VSTO Startup / Shutdown 入口、Application event subscription の呼び出し位置、CustomTaskPane create / remove、shutdown cleanup 呼び出し順序を同時に動かしません。
