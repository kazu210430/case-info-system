...
- application event wiring / unwiring と handler 本体は `VstoEventAdapter` / `ApplicationEventSubscriptionService` 側へ寄り、lifecycle 呼び出し位置は `ThisAddIn` に残しています。
- Startup 周辺は呼び出し順を変えず、`HookApplicationEvents()` と `AddInStartupBoundaryCoordinator.RunAfterApplicationEventsHooked()` の位置を維持します。
- Startup 順序固定メモは `docs/thisaddin-boundary-inventory.md` を参照し、`InitializeStartupBoundaryCoordinator()` -> `InitializeAdaptersAfterComposition()` -> `HookApplicationEvents()` -> `AddInStartupBoundaryCoordinator.RunAfterApplicationEventsHooked()` の並びを現行契約として維持します。
...
- Observation-only isolated visibility investigation logging was added around new CASE base-copy / hidden-open / save-window normalize / cleanup close-quit checkpoints. No cause assertion or behavior change is included in this note.
- Observation-only save-normalize statement logging was added around `NormalizeInteractiveWorkbookWindowStateBeforeSave(...)`. No cause assertion or behavior change is included in this note.
- A minimal save-visible-normal visibility A/B experiment was added to test the existing-window / `xlNormal` path by skipping only `window.Visible=True` while preserving the rest of the flow. No cause assertion is included in this note.
- In current human-side verification, the earliest isolated Excel visible transition still points at `prepare-save-visible-normal`, and a 5-run A/B trial that changed only the conditional skip of `window.Visible=True` made the white-Excel flash disappear in all 5 runs. This is an observation result only, not a cause assertion; no save failure, workbook corruption, CASE create failure, TaskPane regress, Kernel HOME regress, or Accounting regress was observed in those 5 runs, and `WindowState`, foreground / ready-show sequencing, and COM lifecycle were not changed by the experiment.
