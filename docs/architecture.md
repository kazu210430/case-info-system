...
- `DocumentExecutionPolicyService` は allowlist / review 関連ファイルを読む互換レイヤーです。
  - 文書作成本線の runtime 実行可否からは切り離されており、現行実装では permissive に動作します。
  - allowlist / review のファイルと診断用途は残っていますが、完全撤去は次フェーズです。
...
- `DocumentExecutionPolicyService` は現状 permissive な互換レイヤーとして残っており、allowlist / review のファイル読取や診断用途を担います。
