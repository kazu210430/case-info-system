...
- `ThisAddIn_Shutdown` は retained hidden app cache の最終 cleanup を `CaseWorkbookOpenStrategy.ShutdownHiddenApplicationCache()` 経由で実行します。これは legacy route そのものではなく retained cache cleanup を指す shutdown API です。
...
- retained hidden app cache cleanup の shutdown API は `ShutdownHiddenApplicationCache()` を正本とします。
