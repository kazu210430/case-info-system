...
- Document 系
  ...
  - `DocumentExecutionPolicyService` は allowlist / review / pilot 関連ファイルを読む互換レイヤーですが、現行実装では permissive に動作しており、runtime の実行可否そのものは制御しません。
...
- `review`
  ...
  - 今後の段階的撤去候補として扱います。
- `pilot`
  - 現状 runtime 本線では未使用です。
  - 将来拡張フックとして残っていますが、現在は撤去候補です。
- `mode`
  - runtime gating 目的ではありません。
  - 現行コードで確認できる主用途は Word warm-up 制御などの運用スイッチです。
  - allowlist / review / pilot とは分けて扱い、現時点では撤去対象に含めません。

## Document 系サービスの補足

- `DocumentExecutionPolicyService`
  - 現状は permissive な互換レイヤーです。
  - allowlist / review / pilot の過去互換要素を含みます。
  - 将来の段階的撤去対象候補です。
