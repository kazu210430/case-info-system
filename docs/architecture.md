...
この節で固定するのは、close / quit のうち `Kernel HOME close`、`Kernel managed close`、`CASE managed close`、`post-close quit` の到達点だけです。全 workbook close 経路の一般ルールではありません。`KernelUserDataReflectionService` の未 open Base / Accounting 反映は、service-owned な `managed hidden reflection session` の例外として別節で定義します。`MasterWorkbookReadAccessService`、`CaseWorkbookOpenStrategy` などの読み取り専用 / 一時 workbook close は別途棚卸し対象として扱います。

- Form / Service の責務分離
  - `KernelHomeForm` は close の意思表示と `FormClosing` cancel による close 可否制御を担います。
  - `KernelWorkbookService` は HOME session close の backend 調停と finalization、`KernelWorkbookLifecycleService` は Kernel managed close、`CaseWorkbookLifecycleService` は CASE managed close、`PostCloseFollowUpScheduler` は post-close quit を担います。
...
- 既知の残課題
  - helper 非経由の close は他サービスに残っており、今回の確定範囲外です。
...
### 2.1 KernelUserDataReflectionService の例外

- `KernelUserDataReflectionService` の未 open Base / Accounting 反映は、裏 Excel の一律撤去対象ではなく `managed hidden reflection session` として isolated app を許容します。
- owner は `KernelUserDataReflectionService` です。未 open 対象に限り hidden な isolated `Application` を生成し、open した対象 workbook だけを save / close し、最後に生成した `Application` だけを `Quit` します。
- 既に open 済みの Base / Accounting workbook は再利用し、勝手に close しません。
- shared Excel `Application` は `Quit` せず、shared app 側の `DisplayAlerts` / `EnableEvents` / `ScreenUpdating` を残留させません。
- session 中の workbook / window は画面に出しません。
- 目的は、共有中 Excel の window / foreground / lifecycle を変えずに、未 open 対象への反映を owner 付きで閉じることです。

### 2.2 CASE新規作成専用 managed hidden create session

- この例外は `KernelCaseCreationService.CreateSavedCaseWithoutShowing(...)` から `CaseWorkbookOpenStrategy.OpenHiddenWorkbook(...)` を呼ぶ CASE新規作成に限定します。裏 Excel 一般を許容する意味ではありません。
...
- session owner は `KernelCaseCreationService` です。hidden workbook open / close mechanics は `CaseWorkbookOpenStrategy` が担当し、retained app cache を使う場合だけ cached `Application` 自体の owner は `CaseWorkbookOpenStrategy` に残ります。
- interactive モード (`NewCaseDefault` / `CreateCaseSingle`) は hidden create session で save / close を完了したあと、shared app の `OpenHiddenForCaseDisplay(...)` で同じ CASE を reopen し、表示・foreground は `KernelCasePresentationService` と `WorkbookWindowVisibilityService` が担当します。
- `CreateCaseBatch` は hidden create session を close した時点で CASE 作成完了とし、CASE workbook の reopen は行いません。
...
| `experimental-isolated-inner-save` | 専用 hidden `Application` を生成 | なし | cleanup は `legacy-isolated` と同じ | `CASEINFO_EXPERIMENT_DEDICATED_HIDDEN_INNER_SAVE` 指定時の opt-in route です。current/shared app は使わず、差分は close 時の inner save だけです。 |
| `app-cache` | `CaseWorkbookOpenStrategy` cache が専用 hidden `Application` を生成または再利用 | あり | session は workbook close まで担当し、cache owner は idle return / poison / timeout / shutdown で cached `Application` を破棄 | retained instance の唯一の例外です。shared/current app は使いません。 |
...
- `ThisAddIn_Shutdown` は retained hidden app cache の最終 cleanup を `CaseWorkbookOpenStrategy.ShutdownLegacyHiddenApplication()` 経由で実行します。現メソッド名は legacy route そのものではなく retained cache cleanup を指しており、命名 drift が残っています。
- 現コードで保存前に workbook window を明示的に normal / visible へ正規化しているのは `CreateCaseBatch` の `NormalizeBatchWorkbookWindowStateBeforeSave(...)` だけです。interactive route は reopen 後の `OpenHiddenForCaseDisplay(...)` / window recovery で表示を回復しており、「保存ファイルへ hidden window state を残さない」契約は実機確認を伴う別論点として残ります。
...
- `experimental-isolated-inner-save` は current/shared app を使わない dedicated hidden `Application` route として扱います。
- 旧環境変数 `CASEINFO_EXPERIMENT_SHARED_HIDDEN_EXCEL` は互換 alias としてのみ内部対応し、契約上の正本は `CASEINFO_EXPERIMENT_DEDICATED_HIDDEN_INNER_SAVE` とします。
- `ShutdownLegacyHiddenApplication()` は retained hidden app cache cleanup を指しており、route 実態に合わせた命名整理余地があります。
