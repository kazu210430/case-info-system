...
この節の行数、クラスサイズ、`現行` / `現在地` などの current-state 表現は確認時点のスナップショットです。レビュー時の STOP / GO 判定では、`docs/current-state-review.md` に沿って対象ファイルの現行行数、主要責務、差分範囲を実装から再確認します。
...
- `TaskPaneRefreshOrchestrationService`
  ...
  - created-case display session の start / lookup / snapshot 化は `CreatedCaseDisplaySessionStateReader`、emit payload 用材料整形は `TaskPaneRefreshEmitContext` / `TaskPaneRefreshEmitPayloadBuilder`、completion 判定材料と `Blocked` / `SessionMissing` / `ReadyToEmit` 分類は `TaskPaneRefreshCompletionDecisionService` へ分離済みです。
  - retry owner、retry timer lifecycle、ready-show 呼び出し、foreground guarantee 呼び出し、completion callback 発火、emit 発火、created-case display session lifecycle、session dictionary / lock / `IsCompleted` mark / remove、route semantics、callback meaning、UI 表示順序は orchestration 側に残します。
...
- `ThisAddIn`
  - `2026-05-17` の `main` / `origin/main` `616b4af88d15571f0e57a95f447693f0426faeb7` では、`dev/CaseInfoSystem.ExcelAddIn/ThisAddIn.cs` は 645 行です。これは確認時点のスナップショットであり固定仕様ではありません。大型化していた 2187 行規模の状態から、startup / execution boundary と runtime diagnostics を collaborator へ分離した到達点として扱います。
  - VSTO Startup / Shutdown 入口、composition root 呼び出し、Application event hook / unhook の呼び出し位置、adapter delegate surface、CustomTaskPane create / remove adapter、shutdown cleanup 呼び出しは `ThisAddIn` 側に残します。Excel event handler 本体は `VstoEventAdapter` 側へ寄っています。
...
- `KernelWorkbookResolverService`
  - `ResolveOrOpen(...)` / `ResolveOrOpenReadOnly(...)` は pure resolve ではなく、業務都合で必要に応じた temporary open を内包する限定境界です。
  - already-open Kernel workbook を返す場合、workbook ownership は移転しません。呼び出し側はその workbook を勝手に close しません。
  - 未 open Kernel workbook を resolver が current/shared `Application` 上で一時 open した場合だけ、temporary close ownership を `KernelWorkbookAccessResult.CloseIfOwned()` に寄せます。
  - `CloseIfOwned()` は resolver が一時 open した workbook だけを一度だけ閉じ、既に open 済みの Kernel workbook は閉じません。
  - temporary open 後の workbook window は hidden にされますが、これは current app route の一時アクセスであり、専用 hidden `Application` の owner を発生させる経路ではありません。
  - 将来 open 責務を Application Service 側へ寄せる場合も、already-open / newly-opened temporary workbook の ownership 契約、`CloseIfOwned()` の回収境界、hidden/current app route の診断粒度を壊しません。
  - Kernel workbook path、CASE path、`SYSTEM_ROOT` には案件名や個人情報が含まれうるため、診断ログは route / outcome / ownership のような抽象情報を優先し、full path を安易に出しません。

大型クラス削減の目的は、巨大クラスを小さくするだけではなく、変更理由と安全境界を読みやすくすることです。一方で、今回の成果として行数削減と責務削減は実際に進んでいます。

TaskPane completion / emit / session 境界の current-state 契約は `docs/taskpane-display-recovery-current-state.md` を正本とします。次に触る候補は、この境界を動かさない tests / docs 固定、`ThisAddIn` に残る event handler glue のさらなる整理、`CaseWorkbookOpenStrategy` の owner / lifecycle を動かさず切れる追加削減に限ります。実機安定化済みの hidden Excel / white Excel / Book1 / close lifecycle / TaskPane lifecycle は不用意に変更しません。
...
- `TaskPaneSnapshotCacheService`
  ...
  - この参照経路は pure read ではありません。Base 埋込 snapshot の昇格時には CASE の `TASKPANE_SNAPSHOT_CACHE_*` と `TASKPANE_MASTER_VERSION` を DocProperty に書き戻します。
...
- `DocumentNamePromptService` の文書名 prompt は CASE cache-only で、master catalog へフォールバックしません。ただし CASE cache-only lookup の前段で Base snapshot promotion が起きる場合があります。
- 文書実行は CASE cache 優先 + master fallback を維持します。promotion は snapshot/cache/version 整合のための現行副作用であり、文書実行可否や master fallback 順序を変えるものではありません。
...
- `KernelWorkbookResolverService` は CASE workbook の `SYSTEM_ROOT` から Kernel path を解決し、その path と一致する already-open Kernel workbook だけを再利用します。root 不一致を補正するために別 workbook を推測しません。
...
- `SYSTEM_ROOT` 不明、Kernel path 不明、Kernel file 不在の場合は fail-closed の null result とし、workbook は open しません。`Workbooks.Open` 失敗時は現行どおり例外を呼び出し側へ伝播し、`EnableEvents` は復元します。
